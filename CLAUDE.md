...
6. **多模态视觉任务（图片识别、界面 Bug 分析等）** — 优先使用 `ask_chatgpt_mirror` 或 `ask_chatgpt_official` 处理（ChatGPT 支持图片输入，可与代码审查共用同一服务）。`ask_doubao` 作为备用降级选项。

   核心原则：视觉信息不得由 AI 主观臆测。你没有视觉能力，无法看到用户发送的图片。用户口头描述的界面 Bug 常常不完整、不准确。正确的处理流程：

   a. 当用户提供图片并询问界面 Bug 时，**不得自己猜测画面内容**。
   b. 立即将图片作为附件发送给 ChatGPT（mirror 优先），要求**客观描述**图片中的关键视觉信息（文字、按钮状态、错误提示、布局异常等）。
   c. 将返回的描述如实呈现给用户，并请求用户确认：”识别到的画面如上，请问这是否符合实际情况？”
   d. 用户确认后，再根据准确的描述分析问题原因、提供代码修复方案。同一对话可继续发送代码进行审查。

   可委托视觉服务的具体场景：
   - 图片识别与界面 Bug 分析（核心流程）
   - PPT 生成（附大纲/参考文件 → doubao 输出 Markdown，仅 PPT 场景保留 doubao）
   - AI 表格处理（Excel/CSV → 任意服务处理 → 转发结果）
   - 图片/音乐/视频生成提示词
   - 文档阅读与总结（PDF/Word/PPT 附件 → 总结）
   - 编程任务（仅在用户明确要求使用特定服务时）
