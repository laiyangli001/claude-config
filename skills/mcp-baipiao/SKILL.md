...
- [第 5 步：大内容压缩](#第-5-步大内容压缩)
- [第 6 步：加载模板 + 变量插值](#第-6-步加载模板--变量插值)
- [第 7 步：返回结果 + 反馈闭环](#第-7-步返回结果--反馈闭环)
- [第 8 步：日志记录](#第-8-步日志记录)
- [第 9 步：帮助系统](#第-9-步帮助系统)
...
调用 MCP 工具前，如果模板内容或附件文本超过 3000 字符，先用 Headroom 压缩节省 token：
...
如需调整关注点重新执行，**必须重新调用本 skill**（再次输入 `/mcp-baipiao 原需求 + 新关注点`），禁止直接调用 MCP 工具绕过 skill。
