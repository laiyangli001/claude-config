...
| 3 | 重复性任务 | 批量生成/格式化/转换 | deepseek | `batch_task` |
| 4 | 非推理任务 | 文本整理/代码高亮/格式转换 | deepseek | `format_task` |
| 5 | 多模态视觉 | 图片/界面截图/PPT 生成 | doubao | `vision_analysis` |
...
| 多模态 | doubao（图片附件） | deepseek（图片附件） | 文字描述→deepseek | 提示无法处理 |
...
支持 `/mcp-baipiao --history` 查看最近 5 条记录的摘要。
