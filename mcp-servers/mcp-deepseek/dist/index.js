...
// @ts-ignore
import { launchBrowser, navigateWithToast, closeBrowser } from "../../shared/browser.mjs";
// @ts-ignore
...
async function typeAndSend(pg, text) {
    ...
    await el.evaluate((e) => (e.innerText = "")).catch(() => { });
    try {
        await el.fill("");
    }
    catch { }
    const isDiv = await el.evaluate((e) => e.tagName === "DIV" && e.getAttribute("contenteditable") === "true");
    if (isDiv)
        await el.evaluate((e, t) => (e.innerText = t), text);
    else
        await el.fill(text);
