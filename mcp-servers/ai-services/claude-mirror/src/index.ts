...
async function ensureBrowser(): Promise<{ page: Page; context: BrowserContext }> {
  ...
  initPromise = (async (): Promise<{ page: Page; context: BrowserContext }> => {
    ...
    for (const p of existing) {
      try {
        if (!p.isClosed() && p.url().includes("claude.2233.ai") && (await p.locator(SEL.CHAT_INPUT).count()) > 0) {
          page = p; isPageReady = true; found = true; break;
        }
      } catch {}
    }
    if (!found) {
...
async function takeScreenshot(pg: Page, question?: string, attachments?: string[]): Promise<string> {
  ...
  if (question) {
    ...
    for (let i = 0; i < 180; i++) {
      ...
      if (after.length === lastAfterLen) { stable++; if (stable >= 20) break; }
      else { stable = 0; lastAfterLen = after.length; }
    }
  } else {
    await sleep(30000);
  }
...
async function askClaude(question: string, attachments?: string[]): Promise<string> {
  ...
  for (let i = 0; i < 3; i++) {
    ...
    await sleep(200);
    const setText = await chatPg.locator(SEL.CHAT_INPUT).first().evaluate((el: HTMLElement) => el.innerText).catch(() => "");
    if (setText?.includes(question.slice(0, 10))) break;
  }
  ...
  // 等待回答：过滤已知 UI 文字后检测纯回答稳定性
  const qShort = question.slice(0, 10);
  let lastCleanLen = 0, stableCount = 0;
  for (let i = 0; i < 180; i++) {
    await sleep(1000);
    const body = await chatPg.evaluate(() => document.body.innerText);
    const idx = body.lastIndexOf(qShort);
    if (idx < 0) continue;
    // 跳过问题和已知 UI 文字（toast、用量等）
    const after = body.slice(idx + question.length)
      .replace(/⏳.*?\.\.\./g, "")    // toast
      .replace(/用量.*?K/g, "")       // 用量统计
      .replace(/Claude Flex/g, "")    // 页脚
      .trim();
    if (after.length <= 5) continue;  // 还在等待
    if (after.length === lastCleanLen) stableCount++;
    else { stableCount = 0; lastCleanLen = after.length; }
    if (stableCount >= 20) break;     // 纯回答连续 20 秒不变
  }
