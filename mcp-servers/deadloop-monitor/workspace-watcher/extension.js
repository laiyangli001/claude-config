...
function beep() {
  try {
    ...
    if (fs.existsSync(gifPath)) {
      ...
      const hta = '<!DOCTYPE html><hta:application id=o showintaskbar=no caption=no border=none contextmenu=no maximizebutton=no minimizebutton=no><html><head><script>function init(){window.resizeTo(260,180);window.moveTo(screen.availWidth-290,30);setTimeout(function(){window.close()},5000)}<\/script><style>body{margin:0;overflow:hidden;background:#202020;display:flex;width:260px;height:180px}img{width:100%;height:100%;object-fit:contain;cursor:pointer}</style></head><body onload=init()><img id=g src="data:image/gif;base64,' + b64 + '" onclick=close() ondblclick=close()></body></html>';
