...
async function loadRole(roleName: string): Promise<string | null> {
...
async function ensureBrowser(target: string = "mirror") {
  if (browserContext && page && !page.isClosed()) {
    ...
    try {
      ...
      if (hasInput > 0) return { page, context: browserContext };
  ...
  initPromise = (async () => {
    try {
      ...
      try {
        // 重试前再次清理锁文件
        for (const f of ["lockfile", "SingletonLock", "SingletonCookie", "SingletonSocket"]) {
          try { await fs.promises.unlink(path.join(getProfileDir(target), f)); } catch {}
        }
...
async function uploadFilesToChatGPT(page: Page, filePaths: string[]): Promise<void> {
  ...
  for (const fp of filePaths) {
    ...
    try { await fs.promises.access(fp, fs.constants.R_OK); } catch {
...
async function waitForContentStable(page: Page, selector: string): Promise<void> {
  await page.waitForFunction(
    ({ sel, idleMs }: { sel: string; idleMs: number }) => {
    ...
    { sel: selector, idleMs: IDLE_LIMIT_MS },
  ...
  while (retries > 0) {
    try {
      ...
      if (!isPageReady) {
        ...
        let foundPage: Page | null = await findChatPage(browserContext!, target);
        if (foundPage) {
          pg = foundPage;
          page = foundPage;
          isPageReady = true;
...
const server = new Server(
  { name: "chatgpt-mcp", version: "2.0.0" },
  { capabilities: { tools: {} } }
);
