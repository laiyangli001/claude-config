...
function getProfileDir(target: string): string {
  if (process.env.CHATGPT_USER_DATA_DIR) return path.join(process.env.CHATGPT_USER_DATA_DIR, target);
...
async function withLock<T>(fn: () => Promise<T>): Promise<T> {
  return new Promise<T>((resolve, reject) => {
    ...
    const release = () => {
      ...
      released = true;
      lockQueue.shift();
      if (lockQueue.length > 0) lockQueue[0]!();
    };
    ...
    lockQueue.push(async () => {
      try { resolve(await fn()); }
        ...
        clearTimeout(timer);
        if (timedOut) { await closeBrowserResources(); isPageReady = false; }
        release();
      }
    });
    if (lockQueue.length === 1) lockQueue[0]!();
  });
}
...
function log(...args: unknown[]) { if (DEBUG) console.error("[ask_chatgpt]", ...args); }
  ...
  for (let pass = 0; pass < 2; pass++) {
    for (const p of ctx.pages()) {
      try {
        ...
        if (url.includes(targetUrl) && (await p.locator(SEL.CHAT_INPUT).count()) > 0) return p;
      } catch {}
    }
    if (pass === 0) await sleep(500);
  }
  return null;
}

async function ensureBrowser(target: string = "mirror") {
  ...
  return initPromise as Promise<{ page: Page; context: BrowserContext }>;
}

// --- Core ---
async function askChatGPT(question: string, target: string = "mirror", attachments?: string[], role?: string): Promise<string> {
  let retries = 2;
  ...
  while (retries > 0) {
    try {
      // Health check
      if (isPageReady) {
        const pageDead = !page || !browserContext ? true : (() => { try { return page.isClosed(); } catch { return true; } })();
        if (pageDead) { await closeBrowserResources(); isPageReady = false; }
      }
      if (isPageReady && page) {
        try { await Promise.race([page.evaluate(() => 1+1), new Promise((_, r) => setTimeout(() => r(new Error("timeout")), 2000))]); }
        catch {}
      }
      ...
      if (!isPageReady) {
        ...
        if (foundPage) {
          ...
          if ((await startBtn.count()) > 0 && (await startBtn.isVisible())) {
            await startBtn.first().evaluate((el: HTMLButtonElement) => (el.disabled = false));
            await sleep(200);
        ...
        if (!ready) {
          ...
          if (found) { page = found; isPageReady = true; }
          else { await pg.goto(SEL.CHAT_URL, { waitUntil: "domcontentloaded" }).catch(() => {}); isPageReady = false; }
        }
      }
      ...
      if (effectiveRole && effectiveRole !== activeRole) {
        try {
          ...
          if ((await newChatBtn.count()) > 0 && (await newChatBtn.isVisible())) await newChatBtn.click();
        } catch {}
        const tmpl = loadRole(path.resolve(PROJECT_ROOT, "..", "roles"), effectiveRole);
        if (tmpl) {
          await pg.locator(SEL.CHAT_INPUT).first().evaluate((el: HTMLElement) => (el.innerText = tmpl));
      ...
      await pg.locator(SEL.CHAT_INPUT).first().evaluate((el: HTMLElement) => (el.innerText = ""));
      await pg.locator(SEL.CHAT_INPUT).first().evaluate((el: HTMLElement, text: string) => {
        el.innerText = text;
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  if (request.params.name !== "ask_chatgpt") throw new Error("Unknown tool");
  ...
  let target = "mirror";
  if (raw.target === "official" || raw.target === "mirror") target = raw.target;
  ...
  return withLock(async () => {
    try {
      ...
      let pgUrl = "";
      try { if (page && !page.isClosed()) pgUrl = page.url(); } catch {}
